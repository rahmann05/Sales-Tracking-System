...
async function main() {
  ...
  for (const cp of callplans) {
    ...
    // Format nama bersih (misal "SLD539 YULI" -> "Yuli")
    ...
    for (const [dayKey, dayData] of Object.entries(jadwalHarian)) {
      ...
      // Hitung tanggal target berdasarkan hari minggu ini
      ...
      // Simpan Outlets dan siapkan stop data
      ...
      for (let seq = 0; seq < kunjunganList.length; seq++) {
        ...
        // Upsert Outlet berdasarkan customer_id (outletCode)
      ...
      if (stopCreateData.length > 0) {
        // Buat PJP untuk sales tersebut di hari tersebut
