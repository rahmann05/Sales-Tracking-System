 ...
 * 1 File = 1 Component
 */
export const SalesRollingScheduleView = ({ userSchedule = {}, todayDay = 'Senin' }) => {
  return (
    <div className="sales-schedule-container">
      <div>
        <h3 className="text-base font-extrabold text-on-surface flex items-center gap-2">
          ...
          <span>Jadwal Rolling Mingguan Saya (Senin - Sabtu)</span>
        </h3>
      ...
      <div className="sales-schedule-grid">
        {DAYS_LIST.map((day) => {
          ...
          const dayInfo = userSchedule[day] || { clusterName: 'Rolling / Follow-up', outletsCount: 10 };

          return (
            <div
              key={day}
              className={`sales-schedule-day-card ${isToday ? 'active-today' : ''}`}
            >
              ...
              <div className="text-xs font-bold text-on-surface">{dayInfo.clusterName}</div>
              ...
              <div className="flex items-center justify-between text-[11px] text-on-surface-variant mt-auto pt-2 border-t border-border-glass">
                ...
                <span>{dayInfo.subDistrict || 'Bandung Barat'}</span>
              </div>
            </div>
          );
        })}
