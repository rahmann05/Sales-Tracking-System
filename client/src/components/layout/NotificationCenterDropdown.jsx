 ...
 * Fully responsive on both Mobile and Desktop viewports.
 */
  ...
  return (
    <div className="relative">
      <button
        ...
        className="relative p-2 rounded-xl border border-border-glass bg-surface text-on-surface-variant hover:text-on-surface hover:bg-surface-variant transition-all cursor-pointer pointer-events-auto"
        aria-label="Buka Notifikasi"
      >
        <LuBell className="text-lg sm:text-xl" />
        {unreadCount > 0 && (
      ...
      {isOpen && (
        <>
          {/* Backdrop for Mobile */}
          <div 
            className="fixed inset-0 bg-black/30 backdrop-blur-[2px] z-40 sm:hidden pointer-events-auto"
            onClick={() => setIsOpen(false)}
          />

          {/* Responsive Notification Panel */}
          <div className="fixed sm:absolute left-3 right-3 sm:left-auto sm:right-0 top-14 sm:top-full mt-2 w-auto sm:w-96 max-w-sm sm:max-w-none mx-auto sm:mx-0 bg-surface border border-border-glass rounded-3xl shadow-2xl p-4 z-50 space-y-3 max-h-[82vh] flex flex-col animate-in fade-in zoom-in-95 duration-150 pointer-events-auto">
            {/* Header */}
            <div className="flex items-center justify-between border-b border-border-glass pb-3 shrink-0">
              <div>
            ...
            {/* Action Row */}
            <div className="flex items-center justify-between text-[11px] font-bold px-1 text-on-surface-variant shrink-0">
              <button
                type="button"
